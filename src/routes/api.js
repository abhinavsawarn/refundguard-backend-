...
router.post("/auth/login", async (req, res) => {
  try {
    ...
    if (role === "ADMIN") {
      const searchedUser = findUserByUsernameOrEmail(username);
      if (searchedUser && searchedUser.role === "COMPANY") {
        return res.status(400).json({
          error: "This is a Customer account. Please switch the role to 'Customer' above to log in.",
        });
      }
      ...
      if (!isValidAdmin) {
        let record = { count: 1 };
        try { record = recordFailedAttempt(username); } catch (_) {}
        try {
          addLoginLog({
            actor: `ADMIN (${username})`,
            email: (req.headers && req.headers['x-forwarded-for']) || req.ip || "N/A",
            role: "ADMIN",
            result: "FAILED_ATTEMPT",
        ...
        return res.status(401).json({
          ...
          attemptsLeft: Math.max(0, 5 - (record ? record.count : 1)),
        });
      }
      ...
      if (user && user.role === "ADMIN") {
        return res.status(400).json({
          error: "This is an Administrator account. Please select the 'Administrator' role tab above to log in.",
        });
      }
      ...
      if (!user || user.role !== "COMPANY") {
        ...
        return res.status(401).json({
          error: "Invalid customer credentials. (If logging in as admin, switch role to 'Administrator').",
          attemptsLeft: Math.max(0, 5 - (record ? record.count : 1)),
        });
      }
